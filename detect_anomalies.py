...
def detect_anomalies_with_many_stds(ts, normal_model, var_name, ds_name, alg_name, outlier_def='std', standard_deviations=[2,4,8],
                                    plot_save_path=None, data_save_path=None):
                     # TODO: add standard_deviations param (list of 3 floats) and when it isn't None, plot with all of them (red, orangered, orange), save 3 CSVs
    """Detect outliers in the time series data by comparing points against a "normal" model, using a set of three standard deviations as thresholds.
       ...
       Inputs:
           ts [pd Series]:           A pandas Series with a DatetimeIndex and a column for numerical values.
           normal_model [pd Series]: A pandas Series with a DatetimeIndex and a column for numerical values.
           var_name [str]:           The name of the dependent variable in the time series. # TODO: can this come from ts.columns?? (make optional?)
           ds_name [str]:            The name of the time series dataset.
           alg_name [str]:           The name of the algorithm used to create 'normal_model'. # TODO: can this come from normal_model.columns?? (make optional?)
       ...
       Optional Inputs:
           outlier_def [str]:    {'std', 'errors'} The definition of an outlier to be used. Can be 'std' for [num_stds] from the data's mean
                                 ...
                                 Default is 'std'.
           standard_deviations   [list of 3 floats]: Exactly 3 numbers which will be successively used as the number of standard deviations away from the mean that constitutes an outlier. 
                                 Plots will show outliers with all 3 stds and 3 CSVs will be saved.
                                 Default is [2,4,6].
           plot_save_path [str]: The file path (ending in file name) for saving plots of outliers.
           data_save_path [str]: The file path (ending in file name) for saving 
       ...
       Example:
           time_series_with_outliers = detect_anomalies(time_series, model, 'Temperature (C)', 
                                                        'BatteryTemperature', 'ARIMA', 'dynamic',
                                                        plot_path, data_path)
    """
    ...
    outliers3 = pd.Series()
    errors = pd.Series()
    time_series_with_outliers = pd.DataFrame({var_name: ts, alg_name: normal_model})  # TODO: make sure this works, try normal_model.columns for alg_name
    time_series_with_outliers['Outlier'] = 'False'
    ...
    plot_dir = plot_save_path[:plot_save_path.rfind('/')+1]  # TODO: make sure this drops the file name and keeps the slash
    if not os.path.exists(plot_dir): 
        os.makedirs(plot_dir)    
    #pyplot.savefig(plot_save_path, dpi=500)  # TODO: make sure works, Uncomment saving

    pyplot.show()
    pyplot.clf()  # TODO: make sure works

    # Save data
    data_dir = data_save_path[:data_save_path.rfind('/')+1]  # TODO: make sure this drops the file name and keeps the slash
    if not os.path.exists(data_dir): 
        os.makedirs(data_dir)  
    #time_series_with_outliers.to_csv(data_save_path)  # TODO: Uncomment saving
