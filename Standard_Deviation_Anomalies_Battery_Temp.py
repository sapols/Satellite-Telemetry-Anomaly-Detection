...
def parser(x):
    new_time = ''.join(x.split('.')[0])  # remove microseconds from battery temp data
    return datetime.strptime(new_time, '%Y-%m-%d %H:%M:%S')  # for battery temp data


def detect_anomalies_with_mean(ts, num_stds, verbose):
    """Detect outliers in the battery temperature data by comparing points against [num_stds] standard deviations from the mean.

       Inputs:
           ts [pd Series]:   A pandas Series with a DatetimeIndex and a column for temperature.
       ...
       Example:
           battery_temp_with_outliers, outliers = detect_anomalies_with_mean(time_series, 2, True)
       """
    ...
    X = ts.values
    outliers = pd.Series()
    time_series_with_outliers = pd.DataFrame({'Temperature': ts})
    ...
    progress_bar_sliding_window = progressbar.ProgressBar(
        widgets=[progressbar.FormatLabel('Battery Temperature Outliers ')] + widgets,
...
def detect_anomalies_with_rolling_mean(ts, num_stds, window, verbose):
    """Detect outliers in the battery temperature data by comparing points against [num_stds] standard deviations from a rolling mean.
       ...
       Example:
           battery_temp_with_outliers, outliers = detect_anomalies_with_rolling_mean(time_series, 2, window, False)
    """
...
def standard_deviation_anomalies_battery_temp(dataset_path='Data/BatteryTemperature.csv', plots_save_path=None,
                                              verbose=False, use_rolling_mean=False, window=0, num_stds=2):
    """Detect outliers in the battery temperature data by one of the following methods:
       ...
       Optional Inputs:
           dataset_path [str]:      A string path to the battery temperature data. Data is read as a pandas Series with a DatetimeIndex and a column for temperature.
       ...
       Example:
           battery_temp_with_outliers = standard_deviation_anomalies_battery_temp(verbose=True, plots_save_path='Plots/',
    ...
    # time_series = time_series.resample('1H').mean()
    # time_series.to_csv('BatteryTempAveragedHourly.csv')
...
if __name__ == "__main__":
    print('Standard_Deviation_Anomalies_Battery_Temp.py is being run directly')
    battery_temp_with_outliers = standard_deviation_anomalies_battery_temp(verbose=True, use_rolling_mean=False,
                                                                           window=100, num_stds=2)

else:
    print('Standard_Deviation_Anomalies_Battery_Temp.py is being imported into another module')
