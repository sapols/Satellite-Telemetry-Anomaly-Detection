...
def standard_deviation_anomalies_total_bus_current(dataset_path='Data/TotalBusCurrent.csv', plots_save_path=None,
                                             verbose=False):
    """Detect outliers in the total bus current data by comparing points against two standard deviations from the mean.
       ...
       Example:
           bus_current_with_outliers = standard_deviation_anomalies_total_bus_current(verbose=True, plots_save_path='Plots/')
       """
    ...
    outliers = pd.Series()
    time_series_with_outliers = pd.DataFrame({'Bus Current': time_series})
...
if __name__ == "__main__":
    ...
    bus_current_with_outliers = standard_deviation_anomalies_total_bus_current(verbose=True)
